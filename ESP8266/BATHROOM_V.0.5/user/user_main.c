...
LOCAL void ICACHE_FLASH_ATTR dht22();
LOCAL void ICACHE_FLASH_ATTR humiditySetBack_cb();
...
// temporary humidity set (for 30 min fan mode in bathroom)
LOCAL os_timer_t temporaryHumiditySet;
uint32_t tempHumidityTimeout = 15; //temporary fan on for 15 min
int humidityPrevState;
...
if(button_counter==1){
	...
	case 1:
		if (dBR_HUMIDITY_SET != 30) {
			console_printf("Set humidity to 30% for %d minutes\n", tempHumidityTimeout);
			os_timer_disarm(&temporaryHumiditySet);
			os_timer_setfn(&temporaryHumiditySet, (os_timer_func_t *) humiditySetBack_cb, (void *) 0);
			os_timer_arm(&temporaryHumiditySet, tempHumidityTimeout * 60000, 1);
			humidityPrevState = dBR_HUMIDITY_SET;
			dBR_HUMIDITY_SET = 30;
			char tempStr[3];
			os_sprintf(tempStr,"%d", dBR_HUMIDITY_SET);
			MQTT_Publish(&mqttClient, BR_HUMIDITY_SET_CB, tempStr, 2, 0, 0);
			dht22();
		} else {
			humiditySetBack_cb();
		}
...
LOCAL void ICACHE_FLASH_ATTR humiditySetBack_cb() {
	char tempStr[3];
	dBR_HUMIDITY_SET = humidityPrevState;
	os_sprintf(tempStr, "%d", dBR_HUMIDITY_SET);
	console_printf("Set humidity back to %d%%.\n", dBR_HUMIDITY_SET);
	MQTT_Publish(&mqttClient, BR_HUMIDITY_SET_CB, tempStr, 2, 0, 0);
	dht22();
}
